...
const summarizeArgs = (args) => args.map(summarizeArg)
const commandLabel = (input) => [input.command, ...summarizeArgs(input.args)].join(" ")
const timeoutFields = (timeout) => ({
  timeout: Duration.format(timeout),
  timeoutMs: Duration.toMillis(timeout),
})
const commandLogFields = (input, timeout, maxOutputBytes) => ({
...
const commandResultLogFields = (input, timeout, maxOutputBytes, result) => ({
  ...commandLogFields(input, timeout, maxOutputBytes),
  exitCode: result.exitCode,
  stdoutBytes: result.stdout.length,
  stderrBytes: result.stderr.length,
})
const toCommandExecutionError = (input, detail, stderr = "", exitCode = -1) =>
...
const collectOutput = Effect.fn("ProcessRunner.collectOutput")(function* (input, stream, streamName, maxOutputBytes) {
...
const makeProcessRunner = Effect.gen(function* () {
  ...
  const execute = Effect.fn("ProcessRunner.execute")(function* (input) {
