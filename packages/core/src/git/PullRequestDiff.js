...
export const splitDiffByFile = (diffText) => {
  const files = []
...
export const compressChangedLines = (changedLines) => {
...
export const extractHunkHeaders = (patch) =>
  patch
    .split("\n")
    .filter((line) => line.startsWith("@@"))
    .map((line) => line.trim())
const parseDiffPath = (line, prefix) => (line.startsWith(prefix) ? normalizePath(line.slice(prefix.length)) : undefined)
const parseHunkStartLines = (line) => {
...
const getOrCreateLineSet = (map, filePath) => {
...
const extractDiffLineMaps = (diffText) => {
...
export const extractChangedLinesByFile = (diffText) => extractDiffLineMaps(diffText).changedLinesByFile
export const buildTargetRefCandidates = (targetBranch) => {
...
const runGit = (workspace, operation, args, allowNonZeroExit = false) =>
  Effect.gen(function* () {
...
const resolveHeadReviewedSourceCommitCandidate = (workspace) =>
...
export const resolveReviewedSourceCommit = ({ workspace, sourceCommitId }) =>
  Effect.gen(function* () {
...
export const isAncestor = ({ workspace, ancestorRef, headRef }) =>
  Effect.gen(function* () {
...
export const resolveDiffRange = ({ workspace, baseRef, headRef }) =>
  Effect.gen(function* () {
...
export const resolveTargetRef = ({ workspace, targetBranch }) =>
  Effect.gen(function* () {
    if (!targetBranch) {
      return yield* new MissingGitHistoryError({
...
export const resolvePullRequestDiff = (input) =>
  Effect.gen(function* () {
