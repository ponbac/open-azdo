...
export const buildOpenCodeConfig = (agentName) => ({
...
const buildOpenCodeArgs = (request) => [
  "run",
  "--format",
  "json",
  "--agent",
...
export const extractFinalResponse = (output) => {
  const texts = []
  ...
  const maybeCollectStructuredCandidate = (value) => {
...
const makeOpenCodeRunner = Effect.gen(function* () {
  ...
  const run = Effect.fn("OpenCodeRunner.run")(function* (request) {
